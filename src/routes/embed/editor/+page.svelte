<script>
    import { onMount } from "svelte";
    import Authentication from "../../../resources/authentication.js";

    const getLoginDetails = async () => {
        const privateCode = localStorage.getItem("PV");
        if (!privateCode) {
            return { loggedIn: false };
        }

        try {
            const { username } = await Authentication.usernameFromCode(privateCode);
            if (!username) return { loggedIn: false };
            return {
                loggedIn: true,
                username
            };
        } catch (e) {
            console.warn(e);
            return { loggedIn: false };
        }
    };

    onMount(async () => {
        const loginDetails = await getLoginDetails();
        console.log(loginDetails);

        const params = new URLSearchParams(location.search);
        const importLocation = params.get("external");
        if (!importLocation) return console.warn("No external provided");

        const urlObject = new URL(importLocation);
        if (
            !(
                urlObject.host.startsWith("dev.snail-ide.com") || //i havent made this yet but we prob should for snail-ide-dev repos :good:
                urlObject.host.startsWith("editor.snail-ide.com") ||
                urlObject.host.startsWith("snail-ide.com") ||
                urlObject.host.startsWith("localhost:3000")
            )
        ) {
            console.warn("Unsafe host", urlObject.host);
            return;
        }

        // get parent window
        const opener = window.opener || window.parent;
        if (!opener || opener === window) {
            // exit if not found
            console.warn("No parents");
            return;
        }

        // wrapper to handle errors & be easier
        function post(type, data) {
            try {
                opener.postMessage(
                    {
                        type,
                        packet: data,
                    },
                    importLocation
                );
            } catch (e) {
                console.warn("Cannot post message", e);
            }
        }

        post("login", loginDetails);
    });
</script>